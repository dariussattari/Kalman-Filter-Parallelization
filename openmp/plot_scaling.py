#!/usr/bin/env python3
"""Strong + weak scaling figures from the OpenMP results CSVs.

Reads results/strong.csv (fixed M, varying threads) and results/weak.csv
(M scaled with threads), each with header
    impl,M,T,workers,run_idx,wall_time_ms,checksum
takes the median wall_time_ms across runs per (workers, M) combo, and writes:
    <out>/strong_scaling.png   speedup vs threads, log-log, with ideal line
    <out>/weak_scaling.png     parallel efficiency vs threads
...
def median_per_workers(csv_path: str):
    ...
    # cols: 0=impl, 1=M, 2=T, 3=workers, 4=run_idx, 5=wall_time_ms, 6=checksum
...
def main() -> int:
    ...
    ax.plot(workers_s, speedup, "o-", label="measured", lw=1.5, ms=6)
    ax.plot(workers_s, workers_s, "k--", alpha=0.4, label="ideal (linear)")
    ax.set_xlabel("threads")
    ax.set_ylabel(f"speedup vs 1 thread  ($T_1$ = {T1:.0f} ms)")
    ax.set_xscale("log", base=2)
    ax.set_yscale("log", base=2)
    ax.set_xticks(workers_s)
    ax.set_xticklabels([str(w) for w in workers_s])
    ax.set_title(f"OpenMP strong scaling  (M = {M_strong})")
    ax.grid(alpha=0.3, which="both")
    ax.legend()
    # Annotate each measured point with its speedup.
    ...
    M_per_thread = M_w[0] // workers_w[0]

    fig, ax = plt.subplots(figsize=(7.0, 5.0))
    ax.plot(workers_w, efficiency, "o-", label="measured", lw=1.5, ms=6)
    ...
    ax.set_ylim(0, 1.2)
    ax.set_title(f"OpenMP weak scaling  ($M$ per thread = {M_per_thread})")
    ax.grid(alpha=0.3, which="both")
    ax.legend()
