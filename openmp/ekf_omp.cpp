// openmp/ekf_omp.cpp
//
// Part 2 deliverable: OpenMP-parallel Monte-Carlo EKF.
//
// Identical to serial/ekf_serial.cpp except:
//   - #include <omp.h> when compiled with -fopenmp
//   - one #pragma omp parallel for schedule(static) above the realization loop
//   - the worker count comes from omp_get_max_threads() instead of being 1
//   - CSV impl column is "openmp" instead of "serial"
//
// Each thread holds private x[9] and P[81] (stack-allocated inside the loop)
// and its own RNG (seeded by base_seed + m, independent of thread assignment),
// so no critical sections, locks, or reductions are required. checksum_per_m[m]
// is written to a unique index per iteration. Determinism: the total checksum
// is summed in ascending m order in main(), so thread count cannot change the
// final bit pattern.
//
// CLI:
...
void run_ensemble(const ekf_io::Dataset& d, int M, long base_seed,
    ...
    // --- The single OpenMP-specific line in this entire file. ---
    // Without -fopenmp the pragma is silently ignored and the loop runs serially.
...
int main(int argc, char** argv) {
    ...
    if (!a.quiet) {
        std::printf("[ekf_omp] loaded %s: N=%d  T=%.3fs  dt=%.3fms\n",
                    a.data.c_str(), d.N, d.T_seconds, d.dt_imu * 1e3);
        std::printf("[ekf_omp] M=%d  seed=%ld  runs=%d (warmup=%d)  update_period=%d  threads=%d\n",
                    a.M, a.seed, a.runs, a.warmup, ekf_cfg::UPDATE_PERIOD, workers);
    }
    ...
    if (!a.quiet) {
        std::printf("[ekf_omp] median wall = %.2f ms over %d runs\n",
                    median_ms, a.runs);
    }
    ...
    if (!a.out.empty()) {
        ...
        for (int run = 0; run < a.runs; ++run) {
            std::fprintf(f, "openmp,%d,%d,%d,%d,%.6f,%.15g\n",
                         a.M, d.N, workers, run, wall_ms[run], last_total_checksum);
        }
        std::fclose(f);
        if (!a.quiet) std::printf("[ekf_omp] appended %d rows to %s\n", a.runs, a.out.c_str());
    }
    ...
    if (!a.golden.empty()) {
        ...
        std::fclose(f);
        if (!a.quiet) std::printf("[ekf_omp] wrote %s (%d rows)\n", a.golden.c_str(), a.M);
    }
